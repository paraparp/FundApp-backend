...
public class Util {
	...
	public static ExtraData getBloombergData(String url) {
		...
		return new ExtraData(lastPrice, maxPrice, minPrice, updatedAt, null, null, null,null,null,null);
	}
	 */
	...
	public static ExtraData getFTData(String isin) {
		...
		String type = extractTypeFT(ws, categoryFull);
		...
		BigDecimal dailyChangePercent = stringPercentToBigDecimal(ws.scrapDataByXpath(FT_XPATH_DAILY_CHANGE).split("/")[1]);
...
//		BigDecimal fiveYears =  stringPercentToBigDecimal(wp.scrapDataByXpath("/html/body/div[3]/div[3]/section/div[1]/div/div[2]/div[2]/div[2]/table/tbody/tr[1]/td[2]/span"));
		...
		return new ExtraData(name, isinBD, lastPrice, null, null, updatedAt, category, StringUtils.capitalize(type),
				location,ftUrl, dailyChange,dailyChangePercent,oneYear,null);
	}



	private static String extractTypeFT(WebScraper ws, String categoryFull) {
		...
		if (categoryFull.contains("Property")) {
			...
			type=  typeFull.substring(typeFull.lastIndexOf(' ') + 1);
		}
	...
	private static String extractName(WebScraper ws) {
		...
		name = name.replace("Accumulation", "Acc").replace(" Investor", "").replace(" Fund", "").replace("Institutional ", "Inst").trim();
